...
2. **Prerequisites gate — MUST complete before any review work:** For each scope containing files under review, read its `index.md` frontmatter and check for a `follows` field. `_core` Policies always apply to all scopes. Perform ALL of the following checks before proceeding; if ANY check fails, output a FAIL result immediately and do not continue the review:
   ...
   - **Scope-type:** Read the scope's `scope-type` from its `index.md`. If `scope-type` is declared, search the `[type]/principles/` directories of all `core`-type scopes in the workspace for a file whose name ends with `{scope-type}-scope-type.md`. If the scope declares a `scope-type` but no matching file is found, output: `FAIL — Review cannot proceed: scope \`[scope-name]\` declares \`scope-type: [scope-type]\` but no \`[scope-type]-scope-type.md\` policy exists in any \`core\`-type scope in the workspace. The scope is READ-ONLY; install the scope-type governance before reviewing.`
   ...
   - **Scope-type standards (application):** For the scope's declared `scope-type`, load all found companion files (`{scope-type}-scope-type-{qualifier}.md`) from the **same directory** as the primary and apply their rules as additional mandatory conventions alongside the primary. If the primary scope-type policy contains a rule titled `NN-parent-scope-type`, extract the parent type name (backtick-quoted identifier in the rule body) and repeat the search for the parent; continue until no more parents are declared. Detect and stop on cycles. See `_core-adr-policy-010` rules 18 and 20 for the full application model.
   - **Local meta-policies (policy documents only):** Apply all found local meta-policy files as mandatory conventions when reviewing policy documents in the scope. Local meta-policies take highest precedence over scope-type and `follows:` standards on any conflict. See `_core-adr-policy-010` rules 19 and 20.
