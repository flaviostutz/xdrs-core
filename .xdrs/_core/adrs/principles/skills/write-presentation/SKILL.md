...
4. If the user wants slides covering content from multiple documents, check whether an article already exists that synthesizes those documents. If not, suggest creating an article first (using the `write-article` skill) and then creating slides for that article.
...
1. Create the slide file at the correct `.assets/` location:
   ...
   - Skills: `[xdrs-root]/[scope]/[type]/[subject]/skills/[skill-name]/.assets/[slide-file].md`
