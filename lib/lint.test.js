...
test('passes SKILL.md with valid None bullets in Inputs, Outputs, and Halt Conditions', () => {
  ...
  expect(result.errors.join('\n')).not.toContain('must be under 20 words or a single "None"');
});

test('reports SKILL.md bullet over 20 words in #### Required', () => {
  const workspaceRoot = createWorkspace('skill-required-bullet-too-long', {
    ...
    '.xdrs/_local/adrs/principles/skills/check-links/SKILL.md': fullSkillDocument({
      inputsRequired: '- A file path description that is way too long to fit under the twenty word limit because it keeps adding extra words',
    }),
  });

  const result = lintWorkspace(workspaceRoot);

  expect(result.errors.join('\n')).toContain('Skill "### Inputs" -> "#### Required" bullet must be under 20 words');
});
...
test('reports SKILL.md User Interaction bullet over 20 words', () => {
  const workspaceRoot = createWorkspace('skill-user-interaction-bullet-too-long', {
    ...
    '.xdrs/_local/adrs/principles/skills/check-links/SKILL.md': fullSkillDocument({
      userInteraction: '- A clarifying question that is much too long to fit under the twenty word limit because it keeps adding extra words',
    }),
  });

  const result = lintWorkspace(workspaceRoot);

  expect(result.errors.join('\n')).toContain('Skill "### User Interaction" bullet must be under 20 words');
});
...
test.each([[true], [false]])('does not report a missing follows target of an external scope (ignoreExternal=%s) [022.06]', (ignoreExternal) => {
  const workspaceRoot = createWorkspace(`follows-missing-scope-external-${ignoreExternal}`, {
    '.xdrs/index.md': rootIndex(['[myteam](myteam/index.md)']),
    '.xdrs/myteam/index.md': '---\nscope-type: standard\nname: myteam\ndescription: Team.\napply-to: Test team\nvalid-from: 2026-01-01\nfollows: nonexistent-core\n---\n\n# myteam Scope Overview\n\n[ADRs](adrs/index.md)\n',
    '.xdrs/myteam/adrs/index.md': teamAdrIndex(['- [001-team](principles/001-team.md) - Team decision']),
    '.xdrs/myteam/adrs/principles/001-team.md': teamXdrDocument('Team decision.'),
    '.filedist.lock': '.xdrs/myteam/index.md|some-package|1.0.0\n.xdrs/myteam/adrs/index.md|some-package|1.0.0\n.xdrs/myteam/adrs/principles/001-team.md|some-package|1.0.0\n',
  });

  const result = lintWorkspace(workspaceRoot, { ignoreExternal });

  expect(result.errors.join('\n')).not.toContain('nonexistent-core');
  expect(result.errors.join('\n')).not.toContain('022.06');
});

test('does not report a missing extends target of an external scope [022.06]', () => {
  const workspaceRoot = createWorkspace('extends-missing-scope-external', {
    '.xdrs/index.md': rootIndex(['[myteam](myteam/index.md)']),
    '.xdrs/myteam/index.md': '---\nscope-type: standard\nname: myteam\ndescription: Team.\napply-to: Test team\nvalid-from: 2026-01-01\nextends: ghost-lib\n---\n\n# myteam Scope Overview\n\n[ADRs](adrs/index.md)\n',
    '.xdrs/myteam/adrs/index.md': teamAdrIndex(['- [001-team](principles/001-team.md) - Team decision']),
    '.xdrs/myteam/adrs/principles/001-team.md': teamXdrDocument('Team decision.'),
...
test('passes activation checks when only _core is installed and linked (smoke)', () => {
  ...
  expect(Object.keys(result).sort()).toEqual(['errors', 'xdrsRoot']);
});
...
test('does not report an invalid extends-only value in an external scope [022.07]', () => {
...
test('reports an error when no active scope extends an extends-only scope [022.04]', () => {
  ...
  const result = lintWorkspace(workspaceRoot, { ignoreExternal: false });
  expect(activationErrors(result)).toContain('Scope "lib" is tagged `extends-only` but no active scope extends it');
  expect(activationErrors(result)).toContain('add it to the extends: of an active scope or tag it `disabled`');
  expect(activationErrors(result)).toContain('[_core-adr-policy-022.04-extends-only]');
});

test('reports an extends-only scope reached only through a disabled scope, and the frozen extender [022.04, 022.06]', () => {
  ...
  const result = lintWorkspace(workspaceRoot, { ignoreExternal: false });
  expect(activationErrors(result)).toContain('Scope "lib" is tagged `extends-only`');
  expect(activationErrors(result)).toContain('Local scope "myteam" is frozen (read-only) because extends: scope "mid" is tagged `disabled`');
});

test('reports a local scope following a disabled scope as frozen while it still reaches extends-only scopes [022.04, 022.06]', () => {
  ...
  const result = lintWorkspace(workspaceRoot, { ignoreExternal: false });
  const errors = activationErrors(result);
  expect(errors).not.toContain('022.04');
  expect(errors).toContain('Local scope "myteam" is frozen (read-only) because follows: scope "area-core" is tagged `disabled` in the root index; enable it or remove the follows: reference: ');
  expect(errors).toContain('myteam/index.md [_core-adr-policy-022.06-read-only-dependencies]');
});

test('does not report an external scope extending a disabled scope [022.06]', () => {
  ...
  expect(activationErrors(lintWorkspace(workspaceRoot))).toBe('');
  expect(activationErrors(lintWorkspace(workspaceRoot, { ignoreExternal: false }))).toBe('');
});

test('reports _local extending a disabled scope as frozen [022.06]', () => {
  ...
  const result = lintWorkspace(workspaceRoot, { ignoreExternal: false });
  expect(activationErrors(result)).toContain('Local scope "_local" is frozen (read-only)');
});

test('reports a local scope whose type is defined only in a disabled scope as frozen [022.06]', () => {
  ...
  expect(activationErrors(result)).toContain('Local scope "shop" is frozen (read-only) because scope type "widget" is defined only in disabled scope(s) "widgets-core"');
});

test('reports missing dependencies of a disabled local scope as errors [022.05]', () => {
  const workspaceRoot = createWorkspace('activation-disabled-local-missing-deps', {
    '.xdrs/index.md': rootIndex(['[_core](_core/index.md)', '[View scope myteam](myteam/index.md) `disabled`', ACTIVATION_LEGEND]),
    ...scopeFiles('myteam', 'follows: ghost-core\nextends: ghost-lib\n', 'phantom'),
    ...coreWithScopeTypes(['standard']),
  });
  const errors = lintWorkspace(workspaceRoot, { ignoreExternal: false }).errors.join('\n');
  expect(errors).toContain('follows references scope "ghost-core" which does not exist in the workspace');
  expect(errors).toContain('ghost-lib');
  expect(errors).toContain('phantom');
});

test('prints frozen local scopes as errors and never prints warning or READ-ONLY lines [022.06]', () => {
  ...
  const out = [];
  const errOut = [];
  const logSpy = jest.spyOn(console, 'log').mockImplementation((msg) => out.push(msg));
  const errSpy = jest.spyOn(console, 'error').mockImplementation((msg) => errOut.push(msg));
  const warnSpy = jest.spyOn(console, 'warn').mockImplementation((msg) => out.push(msg));
  let code;
  try {
    code = runLintCli(['--all', workspaceRoot]);
  } finally {
    logSpy.mockRestore();
    errSpy.mockRestore();
    warnSpy.mockRestore();
  ...
  expect(code).toBe(1);
  const frozenLines = errOut.filter((l) => l.includes('Local scope "myteam" is frozen (read-only)'));
  expect(frozenLines).toHaveLength(2);
  expect(frozenLines.every((l) => l.includes('[_core-adr-policy-022.06-read-only-dependencies]'))).toBe(true);
  expect([...out, ...errOut].join('\n')).not.toMatch(/\[warning\]|READ-ONLY/);
});

test('silently skips freeze-reference exempt checks [002.01]', () => {
  const frozen = (name) => teamXdrDocument('Frozen decision.')
    .replace(/^---\n/, `---\nfreeze-reference: true\n`)
    .replace(/^name: .*$/m, `name: ${name}`);
  const workspaceRoot = createWorkspace('freeze-reference-silent', {
    '.xdrs/index.md': rootIndex(['[_core](_core/index.md)', '[View scope myteam](myteam/index.md)']),
    '.xdrs/myteam/index.md': scopeIndexWith('myteam'),
    '.xdrs/myteam/adrs/index.md': teamAdrIndex(['- [850-frozen](principles/850-frozen.md) - Frozen']),
    '.xdrs/myteam/adrs/principles/850-frozen.md': frozen('wrong-name'),
  });
  const errors = lintWorkspace(workspaceRoot, { ignoreExternal: false }).errors.join('\n');
  expect(errors).not.toContain('017.05');
  expect(errors).not.toContain('Policy frontmatter name must be');
  expect(errors).not.toContain('Policy title must start with');
});
...
test('dry-run workspace: disabled core, extends-only ref, unlisted legacy scope', () => {
  ...
  const result = lintWorkspace(workspaceRoot, { ignoreExternal: false });
  const errors = activationErrors(result).split('\n');
  expect(errors).toHaveLength(2);
  expect(errors.join('\n')).toContain('[View scope legacy](legacy/index.md)');
  expect(errors.join('\n')).toContain('Local scope "checkout" is frozen (read-only) because follows: scope "ecomm-core" is tagged `disabled`');
...
test('reports missing scope-type for local scopes only [010.10, 022.06]', () => {
  const noType = (name) => `---\nname: ${name}\ndescription: Scope ${name}.\napply-to: Test teams\nvalid-from: 2026-01-01\n---\n\n# ${name}\n`;
...
test('reports missing parent type for local scopes only [010.09, 022.06]', () => {
  ...
  expect(result.errors.join('\n')).not.toMatch(/vendor\/index\.md.*ghost/);
});
