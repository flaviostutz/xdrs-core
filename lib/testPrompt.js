...
function runTaskPhase({ prompt, commandTemplate, workspacePath, timeoutMs, env }, verbose) {
	const wrappedPrompt = [
...
function runJudgePhase({ originalPrompt, judgePrompt, taskOutput, agentReportedChanges, commandTemplate, workspacePath, timeoutMs, env }, verbose) {
	const wrappedPrompt = [
...
function runPromptCommand({ commandTemplate, workspacePath, prompt, timeoutMs, env }, verbose) {
	const command = materializeCommand(commandTemplate, prompt, workspacePath);
...
function materializeCommand(commandTemplate, prompt, workspaceRoot) {
	const materialized = commandTemplate.map((entry) => entry
		.split('{PROMPT}').join(prompt)
		.split('{WORKSPACE_ROOT}').join(workspaceRoot));

	for (let index = 0; index < materialized.length; index += 1) {
		if (materialized[index] === '--add-dir' && materialized[index + 1]) {
			materialized[index + 1] = workspaceRoot;
		}

		if (materialized[index].startsWith('--add-dir=')) {
			materialized[index] = `--add-dir=${workspaceRoot}`;
		}
	}
