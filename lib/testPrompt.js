...
function normalizeConfig(config) {
	...
	return {
		...
		reportFile: config.reportFile ? path.resolve(config.reportFile) : null,
	...
	if (id && options.reportFile) {
		...
		if (entry && entry.result === 'success') {
			...
			if (options.checkOnly) {
				return { passed: false, findings: [{ target: 'workspace', path: 'cache', message: `Test inputs changed since last passing run for id '${id}'. Re-run required.` }] };
			}
		} else if (options.checkOnly) {
			return { passed: false, findings: [{ target: 'workspace', path: 'cache', message: `No passing report entry found for id '${id}'. Re-run required.` }] };
		}
	}

	const result = runPromptTest(config, inputPrompt, judgePrompt, verbose);

	if (result.passed && id && options.reportFile) {
		...
		existingReport[id] = { result: 'success', contextFiles: result.contextFiles, contextHash: newHash };
