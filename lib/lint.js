...
function printHelp() {
  ...
  console.log('Lint the XDR tree rooted at [path] when [path] contains an index.md, or at [path]/.xdrs by default.');
  console.log('\nOptions:');
...
function resolveXdrsRoot(resolvedTarget) {
  // If the path itself contains an index.md, treat it as the XDR root directly.
...
function lintTypeDirectory(xdrsRoot, scopeName, typeName, errors, actualTypeIndexes, externalScopes = new Set()) {
  ...
  for (const entry of entries) {
    ...
    artifacts.push(...lintSubjectDirectory(xdrsRoot, scopeName, typeName, entry.name, xdrNumbers, errors, externalScopes));
  }
...
function lintSubjectDirectory(xdrsRoot, scopeName, typeName, subjectName, xdrNumbers, errors, externalScopes = new Set()) {
  ...
  for (const entry of entries) {
    ...
    lintXdrFile(xdrsRoot, scopeName, typeName, entryPath, xdrNumbers, errors, externalScopes);
  }
...
function lintXdrFile(xdrsRoot, scopeName, typeName, filePath, xdrNumbers, errors, externalScopes = new Set()) {
