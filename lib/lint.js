...
function lintWorkspace(targetPath, options = {}) {
  ...
  lintLocalReadOnlyDependencies(xdrsRoot, scopeMeta, activation, externalScopes, knownScopeTypes, scopeTypeDefiningScopes, errors);

  // Populated during lintScopeIndexFrontmatter: scopeName -> [extendedScopeNames...]
  const extendsRelationships = new Map();

  for (const scopeEntry of scopeEntries) {
    lintScopeDirectory(xdrsRoot, scopeEntry.name, errors, actualTypeIndexes, ignoreExternal, effectiveExternalScopes, knownScopeTypes, extendsRelationships, externalScopes);
  }
  ...
  if (!existsFile(rootIndexPath)) {
    ...
    lintRootIndex(rootIndexPath, xdrsRoot, errors, effectiveExternalScopes, { activation, scopeMeta, knownScopeTypes });
  }

  return { xdrsRoot, errors };
}

function lintRootIndex(rootIndexPath, xdrsRoot, errors, externalScopes, ctx) {
  ...
  for (const [scopeName, entry] of activation) {
    if (entry.tag === 'extends-only' && !reached.has(scopeName)) {
      errors.push(`Scope "${scopeName}" is tagged \`extends-only\` but no active scope extends it, so its policies never apply; add it to the extends: of an active scope or tag it \`disabled\`: ${display} [_core-adr-policy-022.04-extends-only]`);
    }
  }
}
...
// Reports local scopes that would be READ-ONLY per _core-adr-policy-022.06 because of a disabled or
// invalid dependency. Missing dependencies are reported by lintScopeIndexFrontmatter; external scopes are never reported.
function lintLocalReadOnlyDependencies(xdrsRoot, scopeMeta, activation, externalScopes, knownScopeTypes, scopeTypeDefiningScopes, errors) {
  const isDisabled = (scopeName) => (activation.get(scopeName) || {}).tag === 'disabled';
  for (const [scopeName, meta] of scopeMeta) {
    if (!meta || externalScopes.has(scopeName)) continue;
    const causes = [];
    ...
    for (const scopeType of resolveScopeTypeChain(meta.scopeTypes, knownScopeTypes)) {
      if (scopeType === '_local' || !knownScopeTypes.has(scopeType)) continue;
    ...
    const display = toDisplayPath(path.join(xdrsRoot, scopeName, 'index.md'));
    for (const cause of causes) {
      errors.push(`Local scope "${scopeName}" is frozen (read-only) because ${cause}: ${display} [_core-adr-policy-022.06-read-only-dependencies]`);
    }
  }
}
      ...
      for (const entry of entries) {
        ...
        if (SCOPE_NAME_PATTERN.test(entry)) {
          ...
          if (!existsFile(followedIndexPath)) {
            // A missing dependency of an external scope makes it READ-ONLY (_core-adr-policy-022.06); lint does not report it.
            if (!isExternal) {
...
function lintScopeDirectory(xdrsRoot, scopeName, errors, actualTypeIndexes, ignoreExternal, externalScopes, knownScopeTypes = new Map(), extendsRelationships = new Map(), trackedExternalScopes = new Set()) {
  const scopePath = path.join(xdrsRoot, scopeName);
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      lintTypeDirectory(xdrsRoot, scopeName, entry.name, errors, actualTypeIndexes, externalScopes, isCoreTypeNonExempt, isCompiledScope);
...
function lintTypeDirectory(xdrsRoot, scopeName, typeName, errors, actualTypeIndexes, externalScopes = new Set(), enforceStructured = false, isCompiledScope = false) {
  ...
  for (const entry of entries) {
    ...
    if (!ALLOWED_SUBJECTS[typeName].has(entry.name)) {
      ...
      // All policies are frozen (_core-adr-policy-002.01): skip the check, keep content-level checks.
    }

    artifacts.push(...lintSubjectDirectory(xdrsRoot, scopeName, typeName, entry.name, xdrsNumbers, errors, externalScopes, enforceStructured, isCompiledScope));
  }
...
function lintSubjectDirectory(xdrsRoot, scopeName, typeName, subjectName, xdrsNumbers, errors, externalScopes = new Set(), enforceStructured = false, isCompiledScope = false) {
  ...
  for (const entry of entries) {
    ...
    lintXdrsElementFile(xdrsRoot, scopeName, typeName, subjectName, entryPath, xdrsNumbers, errors, externalScopes, enforceStructured, isCompiledScope);
  }
...
function lintXdrsElementFile(xdrsRoot, scopeName, typeName, subjectName, filePath, xdrsNumbers, errors, externalScopes = new Set(), enforceStructured = false, isCompiledScope = false) {
  ...
  lintPolicyNumberRange(number, subjectName, content, filePath, errors);
  ...
  if (!freezeReference && titleParts.includes('core') && !(subjectName === 'principles' && titleParts[0] === 'core')) {
    errors.push(`Policy filename must not use "core" as a name word unless it is a local meta-policy in "principles/" starting with "core" (e.g., "NNN-core.md" or "NNN-core-{qualifier}.md"): ${toDisplayPath(filePath)} [_core-adr-policy-010.13-local-naming]`);
  }

  const expectedName = extractExpectedXdrsNameFromHeading(firstLine)
    || `${scopeName}-${TYPE_TO_ID[typeName]}-${number}-${match[2]}`;
  lintXdrsElementFrontmatter(content, expectedName, filePath, errors, freezeReference);
...
function lintXdrsElementFrontmatter(content, expectedName, filePath, errors, freezeReference = false) {
  const fm = extractFrontmatter(content);
  if (!fm.present) {
  ...
  if (!fm.name) {
    ...
    if (fm.name !== expectedName && !freezeReference) {
 ...
 * bullets each under 20 words, per _core-adr-policy-003. No-op when the range has no bullets
...
function lintBulletWordCountSection(lines, ignoredLines, start, end, label, filePath, errors) {
  ...
  for (const bullet of bullets) {
    ...
    if (wordCount >= 20) {
      errors.push(`${label} bullet must be under 20 words or a single "None" (found ${wordCount}): ${toDisplayPath(filePath)} [_core-adr-policy-003]`);
    }
  }
}

/**
 * Validates the item word-count rule (single "None" bullet, or one-or-more sub-20-word
...
function lintPolicyNumberRange(number, subjectName, content, filePath, errors) {
  if (extractFrontmatter(content).freezeReference) return;
  const num = parseInt(number, 10);
  if (num >= RESERVED_NUMBER_RANGE_MIN && num <= RESERVED_NUMBER_RANGE_MAX) {
    errors.push(`Policy number ${number} is in the reserved range (801–900) which MUST NOT be assigned: ${toDisplayPath(filePath)} [_core-adr-policy-017.05-reserved-gap-must-not-be-assigned]`);
    return;
  }
  if (num < OVERFLOW_RANGE_MIN) {
    const range = SUBJECT_NUMBER_RANGES[subjectName];
    if (range && (num < range[0] || num > range[1])) {
      errors.push(`Policy number ${number} is outside the ${range[0]}–${range[1]} block reserved for subject "${subjectName}": ${toDisplayPath(filePath)} [_core-adr-policy-017.01-numbering-blocks-per-subject]`);
    }
  }
}
...
function lintDocumentLinks(documentPath, xdrsRoot, scopeName, errors, externalScopes = new Set()) {
  ...
  const isSkillFile = path.basename(documentPath) === 'SKILL.md' && path.basename(path.dirname(documentDir)) === 'skills';
  const sharedSkillResourceDir = isSkillFile ? path.join(path.dirname(documentDir), RESOURCE_DIR_NAME) : null;
  const localScopePath = normalizePath(path.join(xdrsRoot, '_local'));
  const repoRoot = path.dirname(xdrsRoot);
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (const link of parseLocalLinkTargets(lines[index], documentDir, repoRoot)) {
      ...
      if (isResourceLink && !isPathInside(resourceDir, link.resolvedPath)
        && !(sharedSkillResourceDir && isPathInside(sharedSkillResourceDir, link.resolvedPath))) {
