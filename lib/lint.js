...
function lintSourceSnapshotTracking(sourcesDir, errors) {
  ...
  for (const entry of entries) {
    ...
    if (!existsFile(trackingFilePath)) {
      errors.push(`Compiled source snapshot folder must contain a "${SOURCE_TRACKING_FILE_NAME}" tracking file: ${toDisplayPath(trackingFilePath)} [_core-adr-policy-019.12-source-storage-tracking]`);
      continue;
    }
...
function validateSourceTrackingFile(filePath, errors) {
  ...
  if (firstNonEmptyLine(content) !== '# Source') {
    errors.push(`Source tracking file must start with a "# Source" heading: ${toDisplayPath(filePath)} [_core-adr-policy-019.12-source-storage-tracking]`);
  }
  ...
  for (const field of ['last-fetch-timestamp', 'last-compilation-timestamp']) {
    ...
    if (!fieldLine) {
      errors.push(`Source tracking file must include a "${field}:" line: ${toDisplayPath(filePath)} [_core-adr-policy-019.12-source-storage-tracking]`);
      continue;
    }
    const value = fieldLine.trim().slice(`${field}:`.length).trim();
    if (!timestampRe.test(value)) {
      errors.push(`Source tracking file "${field}" must match "YYYY-MM-DD HH:MM:SS": ${toDisplayPath(filePath)} [_core-adr-policy-019.12-source-storage-tracking]`);
    }
  }
}
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (const link of parseLocalLinkTargets(lines[index], documentDir, repoRoot)) {
      ...
      if (isSourceSnapshotPath(link.resolvedPath)) {
        errors.push(`Document must not link directly to compiled-source snapshot content, which is not authoritative (see _core-adr-policy-019 rule 14) in ${toDisplayPath(documentPath)}:${index + 1}: ${link.rawTarget} [_core-adr-policy-019.14-source-not-authoritative]`);
      }
    }
  }
}
